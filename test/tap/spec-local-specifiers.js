...
test('specifiers', function (t) {
  ...
  installOk.forEach(function (mod) {
    t.test(mod, function (t) {
      common.npm(['install', '--dry-run', '--json', mod], conf, function (err, code, stdout, stderr) {
        if (err) throw err
        t.is(code, 0, 'command ran ok')
        t.comment(stdout.trim())
        if (stderr) t.comment(stderr.trim())
        t.done()
      })
    })
  })
  slashes.forEach(function (os) {
    t.test('win-abs-drive-' + os, function (t) {
      common.npm(['install', '--dry-run', '--json', 'win-abs-drive-' + os], conf, function (err, code, stdout, stderr) {
        if (err) throw err
...
test('ambiguity', function (t) {
  ...
  t.test('arg: looks like package name, is dir', function (t) {
    common.npm(['install', 'mkdirp', '--dry-run', '--json'], conf, function (err, code, stdout, stderr) {
      if (err) throw err
      t.is(code, 0, 'command ran ok')
      if (stderr) t.comment(stderr.trim())
...
v  })
  t.test('arg: looks like package name, is package', function (t) {
    common.npm(['install', 'wordwrap', '--dry-run', '--json'], conf, function (err, code, stdout, stderr) {
      if (err) throw err
      t.is(code, 0, 'command ran ok')
      if (stderr) t.comment(stderr.trim())
      const result = JSON.parse(stdout.trim())
  ...
  t.test('arg: looks like github repo, is dir', function (t) {
    common.npm(['install', 'example/minimist', '--dry-run', '--json'], conf, function (err, code, stdout, stderr) {
      if (err) throw err
      t.is(code, 0, 'command ran ok')
      if (stderr) t.comment(stderr.trim())
      const result = JSON.parse(stdout.trim())
  ...
  t.test('package: looks like tag, has dir, use tag', function (t) {
    common.npm(['install', '--dry-run', '--json'], {cwd: path.join(testdir, 'prefer-pkg'), env: conf.env}, function (err, code, stdout, stderr) {
      if (err) throw err
      t.is(code, 0, 'command ran ok')
      if (stderr) t.comment(stderr.trim())
      t.comment(stdout.trim())
...
test('existing install matches', function (t) {
  ...
  t.test('relative symlink counts as match of relative symlink in package.json', function (t) {
    common.npm(['ls', '--json'], {cwd: path.join(testdir, 'existing-matches'), env: conf.env}, function (err, code, stdout, stderr) {
      if (err) throw err
      t.is(code, 0, 'command ran ok')
...
test('install behavior', function (t) {
  ...
  t.test('transitive deps for in-larger-module cases', function (t) {
    common.npm(['install', 'file:ib-in'], {cwd: ibdir, env: conf.env}, function (err, code, stdout, stderr) {
      if (err) throw err
      t.is(code, 0, 'command ran ok')
      if (stdout) t.comment(stdout.trim())
      if (stderr) t.comment(stderr.trim())
  ...
  t.test('transitive deps for out-of-larger module cases', function (t) {
    common.npm(['install', 'file:../ib-out'], {cwd: ibdir, env: conf.env}, function (err, code, stdout, stderr) {
      if (err) throw err
      t.is(code, 0, 'command ran ok')
      if (stdout) t.comment(stdout.trim())
      if (stderr) t.comment(stderr.trim())
  ...
  t.test('transitive deps for out-of-larger module cases w/ --preserve-symlinks', function (t) {
    ...
    common.npm(['install', 'file:../ib-out'], {cwd: ibdir, env: env}, function (err, code, stdout, stderr) {
      if (err) throw err
      t.is(code, 0, 'command ran ok')
      if (stdout) t.comment(stdout.trim())
      if (stderr) t.comment(stderr.trim())
  ...
  t.test('tar/tgz/tar.gz should install a tarball', function (t) {
    common.npm(['install', 'file:tarball-1.0.0.tgz'], {cwd: ibdir, env: conf.env}, function (err, code, stdout, stderr) {
      if (err) throw err
      t.is(code, 0, 'command ran ok')
      if (stdout) t.comment(stdout.trim())
      if (stderr) t.comment(stderr.trim())
  ...
  t.test('non tar/tgz/tar.gz files should give good error message', function (t) {
    common.npm(['install', 'file:noext'], {cwd: ibdir, env: conf.env}, function (err, code, stdout, stderr) {
      if (err) throw err
  ...
  t.test('directories without package.json should give good error message', function (t) {
    common.npm(['install', 'file:not-module'], {cwd: ibdir, env: conf.env}, function (err, code, stdout, stderr) {
      if (err) throw err
  ...
  t.test('verify preinstall step runs after finalize, such that cwd is as expected', function (t) {
    common.npm(['install', 'file:test-preinstall'], {cwd: ibdir, env: conf.env}, function (err, code, stdout, stderr) {
      if (err) throw err
      t.is(code, 0, 'command ran ok')
...
test('save behavior', function (t) {
  t.plan(4)
  t.test('to package.json and npm-shrinkwrap.json w/ abs')
  t.test('to package.json and npm-shrinkwrap.json w/ drive abs')
  t.test('to package.json and npm-shrinkwrap.json w/ rel')
  t.test('transitive dependencies of shrinkwraps')
})
